...
export function createWriteMode(options: CreateWriteModeOptions): WriteMode {
  ...
  function assertAllowed(absolutePath: string): void {
    if (!isPathAllowed(absolutePath, root, allowPaths)) {
      throw Object.assign(
        new Error(`Path not allowlisted: ${absolutePath}`),
  ...
  return {
    ...
    async upsertEntry(entry: ContentEntry) {
      ...
      await writer.writeText(absolutePath, `${JSON.stringify(next.data, null, 2)}\n`);

      const list = catalog[entry.collection] ?? (catalog[entry.collection] = []);
    ...
    async deleteEntry(collection: string, id: string) {
      ...
      catalog[collection] = (catalog[collection] ?? []).filter((e) => e.id !== id);
    },
  };
}
