/**
 * PROTOTYPE / THROWWAWAY — Deno Desktop control shell.
 *
 * Assumption: orchestration/logic prototype (not UI variants). Answers:
 * "Does clone → install → run → native webview preview feel right for
 * validating a GitHub repo from a desktop control shell?"
 *
 * Run: bun run prototype:clone-run
 *
 * Uses Deno Desktop native WebView (WKWebView on macOS) — not Playwright/CEF.
 * Docs: https://docs.deno.com/runtime/desktop/
 */
...
const HERE = join(Deno.cwd(), ".scratch/prototype-clone-run");
const WIP_ROOT = join(HERE, "WIP-PROTOTYPE-wipe-me");
...
async function runPipeline(repoUrl: string, subdirectory = "") {
	...
	try {
		...
		const devCmd = resolveDevCommand(dir, pm);
		dispatch({
			type: "detected",
			packageManager: pm,
			workDir: dir,
		...
		dispatch({ type: "log", line: `install via ${pm}` });
		const install = await runCaptured(installCommand(pm), {
			cwd: dir,
			onLine: (line) => {
				if (token !== runToken) return;
				dispatch({ type: "log", line });
		...
		if (token !== runToken) return;
		if (!install.success) {
			throw new Error(install.stderr || install.stdout || "install failed");
		}
...
<title>Clone → Run prototype</title>
<style>
  :root {
  ...
  * { box-sizing: border-box; }
  ...
  .grid { display: grid; grid-template-columns: 140px 1fr; gap: 0.35rem 0.75rem; }
...
<body>
<main>
  <div class="badge">PROTOTYPE · Deno Desktop · native WebView</div>
  <h1>Clone → install → run → preview</h1>
  <p class="q">Does this orchestration feel right for validating a GitHub repo from a desktop control shell? Watch the state panel after every action. Preview opens in a second native webview window.</p>
...
const scenarios = [
  {
    id: "happy",
    name: "Happy path",
    blurb: "Clone Astro's blog example (Zod content collections via content.config.ts), flatten the monorepo subdir, install, start dev, open native webview preview.",
    steps: [
  ...
  },
  {
    ...
    blurb: "Kick off the happy path, then stop before preview. Confirms teardown of the child process.",
    steps: [
...
document.getElementById("btn-copy-logs").onclick = async () => {
  ...
  try {
    ...
    // Fallback if binding unavailable
    try {
...
function bindWindow(win: DesktopWindow) {
	...
	win.bind("inspectSchemas", async () => {
		...
		const script = join(
			Deno.cwd(),
			".scratch/prototype-clone-run/inspect-content-config.mjs",
		);
		dispatch({ type: "log", line: `inspect schemas in ${state.workDir}` });
		const result = await runCaptured(["node", script, state.workDir], {
			cwd: state.workDir,
...
if (!BrowserWindow) {
	console.error(
		"Deno.BrowserWindow is missing. Start with:\n  bun run prototype:clone-run",
	);
	Deno.exit(1);
}

await Deno.mkdir(WIP_ROOT, { recursive: true });

const controlWin = new BrowserWindow({
	title: "Clone→Run prototype (control)",
	width: 920,
	height: 820,
});
...
<h1>Preview</h1>
<p>Start the pipeline from the control window. This native webview will navigate to the child app’s local URL.</p>
</body>`),
);
...
console.log("PROTOTYPE ready — control + preview native webview windows.");
