...
The form shell uses SJSF to render the selected union branch as a nested part of the same form. FieldUi / catalog bindings inside block schemas continue to apply when blocks return as IR kinds ([#29](https://github.com/bbtgnn/dev-cms/issues/29); historical Zod `.meta()` FieldUi removed by ADR-0020). Add, reorder, duplicate, remove, defaults, and validation are editor behaviors governed by configured capabilities.
