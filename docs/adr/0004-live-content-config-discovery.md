...
For the **filesystem / Astro host adapter**, discover editable collections by Vite-importing the host `src/content.config.*` and reading `export const collections` — the same graph Astro uses. Collection chrome may use root schema `.meta(config({...}))`. No required parallel `cms.config` for that adapter. JSON-Schema digests alone are insufficient when `meta.ui` holds components.
