...
Semantic fields, FieldUi, and recursive layout declarations are the normal authoring interface. Raw SJSF widget, field, and template slots remain an explicit advanced escape hatch; the authoring shell does not mirror the whole SJSF interface as its own public contract.
