...
Stock editors (by stamped kind), optional form-tree chrome / catalog bindings,
and recursive layout declarations are the normal authoring interface. Raw SJSF
widget, field, and template slots remain an explicit advanced escape hatch; the
authoring shell does not mirror the whole SJSF interface as its own public
contract.
...
layout is authored in an optional form tree (or stock default); Zod `.meta()`
FieldUi and CMS-first unified-tree FieldUi are historical
