...
Happy path: `http://127.0.0.1:4321/cms` (shell via `injectRoute`) and `/cms/api`
(JSON API via injected catch-all route).
...
| `/cms/api/collections/posts` | FS-scan entries under `src/content/posts` |
| `/cms/api/collections/authors` | FS-scan entries under `src/content/authors` |
| `/cms/api/collections/posts/hello` | Get JSON entry (includes `author` string id) |
| `PUT /cms/api/collections/posts/new-post` | Upsert (allowlisted) |
| `DELETE /cms/api/collections/posts/new-post` | Delete (204) |
...
curl -sS -X PUT \
  http://127.0.0.1:4321/cms/api/collections/posts/smoke-temp \
...
REV=$(curl -sS http://127.0.0.1:4321/cms/api/collections/posts/hello | bun -e 'console.log(JSON.parse(await Bun.stdin.text()).revision)')
curl -sS -X PUT \
  http://127.0.0.1:4321/cms/api/collections/posts/hello \
  -H 'content-type: application/json' \
