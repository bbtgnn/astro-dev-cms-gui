...
- **FS adapter (v1):** JSON round-trip, id/path rules; default host builds collection descriptors from compiled IR / schema partition (ADR-0004, 0017, 0007, 0019, 0020).
- **FieldUi / schema:** CMS-first unified tree in `cms.config.ts`; Astro `content.config.ts` is generated; components via Vite, not the protocol. Editor configs are IR form models only — no Zod/FieldUi dual path (ADR-0019, 0020; supersedes ADR-0003).
...
- **Blocks / preview:** block schemas stay separate from production renderers; the real Astro page is the default preview (ADR-0012, 0013). Revive blocks/i18n as IR kinds ([#29](https://github.com/bbtgnn/astro-dev-cms-gui/issues/29)), not Zod FieldUi.
...
- **Reference host:** `demos/astro-simple` (`@cms/astro-demo-simple`, default `bun run dev` → `:4321`) and `demos/astro-overlay` (`@cms/astro-demo`, `bun run dev:overlay` → `:4322`); shell `/cms`, API `/_cms` via `@cms/astro`. Libraries stay under `packages/`.
